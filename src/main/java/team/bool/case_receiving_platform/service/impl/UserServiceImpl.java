...
public class UserServiceImpl implements UserService {
	...
	private AuthRes checkUserEmailAndPwd(String email, String pwd) {
		
		// input is null & check input format
		if (!StringUtils.hasText(email) || !email.matches(emailPattern)) {
			return new AuthRes(AuthRtnCode.EMAIL_FORMAT_ERROR.getCode(),
		...
		// Account not found
		if (op.isEmpty()) {
			return new AuthRes(AuthRtnCode.ACCOUNT_NOT_FOUND.getCode(),
		...
		try {
			...
			return new AuthRes(AuthRtnCode.SUCCESSFUL.getCode(), AuthRtnCode.SUCCESSFUL.getMessage(),
					userInfo);
